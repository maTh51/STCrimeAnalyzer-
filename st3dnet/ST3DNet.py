...
class Recalibration(Layer):
    ...
    def call(self, x):
        double_x = tf.stack(
            [x, x], axis=2
        )  # [(batch,c,h,w), (batch, c,h,w)] => (batch,c,2,h,w)
...
class Recalibration_T(Layer):
    ...
    def call(self, x):
        ...
        W = tf.reshape(
            tf.tile(self.W, [map_height * map_width]),
            (nb_channel, 2, map_height, map_width),
        )  # tile:sharing channel-wsie weight on different positions in the weekly-period recalibration block
        double_x = tf.stack(
            [x, x], axis=2
        )  # stack [(batch, c,h, w)] = (batch, c, 2, h,w)
...
def ST3DNet(
    c_conf=(6, 2, 16, 8), t_conf=(4, 2, 16, 8), external_dim=8, nb_residual_unit=4
):
    ...
    if len_closeness > 0:
        input = Input(
            shape=(nb_flow, len_closeness, map_height, map_width)
        )  # (2,t_c,h,w)
        ...
        # Conv2 3D
        conv = Conv3D(
            filters=64, kernel_size=(3, 3, 3), strides=(3, 1, 1), padding="same"
        )(conv)
        conv = Activation("relu")(conv)
        ...
        residual_output = ResUnits(
            _residual_unit, nb_filter=64, repetations=nb_residual_unit
        )(reshape)
