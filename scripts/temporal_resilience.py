...
if __name__ == '__main__':
    ...
    path_save = sys.argv[2] if len(sys.argv) > 1 else './scripts/results/temporal_resilience'
    ...
    if 'STHSLModel' in config['models'].keys():
        config['models']['STHSLModel']['last_test_date'] = test_end_date            
    ...
    for _ in range(steps):
        ...
        for m, params in config['models'].items():
            ...
            if 'slide_window' in params.keys():
                ...
                model = globals()[m](points=points, grid = grid, last_train_date = config['evaluation']['train_end_date'], temporal_granularity=config['evaluation']['temporal_granularity'], static = True, **params)
            else:
