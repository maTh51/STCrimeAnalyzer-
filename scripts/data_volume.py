...
if __name__ == '__main__':
    ...
    path_save = sys.argv[2] if len(sys.argv) > 1 else './scripts/results/data_volume'
    ...
    for _ in range(steps):
        ...
        for data_volume in range(1, 11, 1):
            ...
            print(f'Train with {data_volume*10}% of data...')
            ...
            for m, params in config['models'].items():
                ...
                if 'slide_window' in params.keys():
                    ...
                    model = globals()[m](points=points.copy(), grid = grid, last_train_date = config['evaluation']['train_end_date'], temporal_granularity=config['evaluation']['temporal_granularity'], data_volume=data_volume/10, **params)
                else:
                    model = globals()[m](points=train_points.copy(), grid = grid, last_train_date = config['evaluation']['train_end_date'],temporal_granularity=config['evaluation']['temporal_granularity'],**params)
                model.train()
                models.append(model)
            # %%
            #Evaluation
        ...
        tmp = tmp.groupby(['data_volume','model']).mean().reset_index()
        results.append(tmp)
